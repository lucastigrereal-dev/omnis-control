...
Ponto crítico de integridade. Decisão: **Postgres é a fonte de verdade dos IDs (source of truth).** Toda entidade nasce no Postgres com um `uuid`. Qdrant e Neo4j usam ESSE mesmo `uuid` como chave externa — nunca geram ID próprio. Escrita é transacional via uma camada única (`src/memory/interface.py` será o ÚNICO ponto de escrita nos três — **⚠️ Wave A 2026-05-22: `src/providers/` NÃO EXISTE no disco; quando implementado, o módulo canônico ficará em `src/memory/` ou `src/multi_model_orchestration/`, não em `src/providers/`**). Nada escreve direto em Qdrant ou Neo4j sem passar por lá. Criar T-204 com esse contrato: um write = um uuid propagado aos três, ou rollback nos três. Sem fonte única de ID, vira três cérebros com memórias divergentes.
